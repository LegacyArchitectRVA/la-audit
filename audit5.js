  ...
  window.__la = {
    ...
    sub: function() {
      ...
      xhr.onload=function(){if(xhr.status>=200&&xhr.status<300){if(msg)msg.textContent='Thank you. Your results are on the way.';em.value='';em.style.borderColor='#b8984e';}else{if(msg)msg.textContent='Something went wrong. Please try again.';}};
      xhr.onerror=function(){if(msg)msg.textContent='Connection error. Please try again.';};
      xhr.send(JSON.stringify({fields:[{objectTypeId:'0-1',name:'email',value:email},{objectTypeId:'0-1',name:'message',value:body}]}));
    },
