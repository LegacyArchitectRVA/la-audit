...
(function(){
    ...
    if(isP5){
      gate='<div style="margin-top:36px;padding-top:32px;border-top:1px solid #2a2218;margin-bottom:52px;">'+
        ...
        '<div style="display:flex;gap:12px;">'+
          ...
          '<button id="bn" onclick="__la.bn()" style="font-family:Cinzel,serif;font-size:14px;font-weight:700;letter-spacing:3px;padding:13px 32px;border:1px solid '+(OB===false?'#6b5a38':'#4a3d28')+';background:'+(OB===false?'rgba(78,58,25,0.08)':'transparent')+';color:#8a7240;cursor:pointer;border-radius:1px;box-shadow:'+(OB===false?'0 0 18px rgba(107,90,56,0.5),inset 0 0 12px rgba(107,90,56,0.08)':'none')+';">NO</button>'+
        '</div>'+
  ...
  window.__la={
    ...
    bn:function(){OB=false;var y=document.getElementById('by'),n=document.getElementById('bn'),h=document.getElementById('bh');if(n){n.style.borderColor='#6b5a38';n.style.background='rgba(78,58,25,0.08)';n.style.boxShadow='0 0 18px rgba(107,90,56,0.5),inset 0 0 12px rgba(107,90,56,0.08)';}if(y){y.style.borderColor='#4a3d28';y.style.color='#8a7240';y.style.background='transparent';y.style.boxShadow='none';}if(h){h.textContent='Your score will be calculated across 6 pillars.';h.style.color='#9a8d7a';}},
    ...
    sub:function(){
      ...
      xhr.onload=function(){if(xhr.status>=200&&xhr.status<300){var sec=document.getElementById('la-email-sec');if(sec){var tot2=0,u2=0;for(var ri=0;ri<7;ri++){if(ri===5&&OB===false)continue;ST[ri].forEach(function(v){tot2+=v;});u2++;}var pct2=Math.round(tot2/(u2*6)*100);var mx2=u2*6;var t2,td2,tr2;if(tot2<=10){t2='LEAN & READY';td2='Minimal digital and legal silos. A standard continuity plan would cover this.';tr2='Most people in this range start with <strong>The Vault</strong>.';}else if(tot2<=22){t2='LEGACY AT RISK';td2='Some documentation exists, but access gaps and single points of failure remain.';tr2='Most people in this range benefit from <strong>The Archive</strong>.';}else if(tot2<=30){t2='CRITICAL COMPLEXITY';td2='High-value, multi-layered responsibilities. Risk of permanent asset loss without a clear system.';tr2='Most people in this range need <strong>The Legacy</strong>.';}else if(tot2<=36){t2='WELL STRUCTURED';td2='Strong foundation in place. A few remaining gaps to close.';tr2='A focused session can identify what is missing.';}else{t2='COMPREHENSIVE';td2='Thorough documentation across all pillars. Successor readiness is high.';tr2='An annual review keeps this current.';}sec.innerHTML='<div style="font-family:Bodoni Moda,serif;font-size:16px;font-style:italic;color:#b8984e;margin-bottom:20px;">Thank you. Here are your full results.</div>'+'<div style="text-align:center;margin-bottom:28px;">'+'<div style="font-family:Cinzel,serif;font-size:36px;font-weight:600;color:#c1b085;text-shadow:0 0 20px rgba(193,176,133,0.3);">'+pct2+'%</div>'+'<div style="font-family:Cinzel,serif;font-size:14px;letter-spacing:3px;color:#c1b085;margin-top:4px;">'+tot2+' OF '+mx2+' POINTS</div>'+'<div style="font-family:Cinzel,serif;font-size:18px;font-weight:700;color:#c1b085;letter-spacing:2px;margin-top:8px;">'+t2+'</div>'+'<div style="font-family:Bodoni Moda,serif;font-size:15px;font-style:italic;color:#b0a494;line-height:1.5;margin-top:8px;">'+td2+'</div>'+'<div style="font-family:Bodoni Moda,serif;font-size:14px;font-style:italic;color:#b8984e;line-height:1.5;margin-top:12px;">'+tr2+'</div>'+'</div>'+detailHTML();}}else{if(msg)msg.textContent='Something went wrong. Please try again.';}};
