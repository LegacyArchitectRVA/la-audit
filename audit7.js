(function () {
  ...
  function updatePage1State() {
    for (var i = 0; i < 6; i++) {
      ...
      var wrap = cb ? cb.closest(".la-item-wrap") : null;
      if (wrap) {
        if (ST[0][i] === -1) wrap.classList.add("is-na");
  ...
  function updatePage1Counter() {
    var ctr = el("la-ctr-0");
    if (!ctr) return;
    var box = ctr.firstElementChild;
    if (!box) return;
    var spans = box.querySelectorAll("span");
    if (spans.length < 2) return;

    var c = checkedCount(0);
    var a = activeCount(0);

    spans[0].textContent = c;
    spans[1].textContent = "of " + a;

    if (c > 0) {
      box.style.borderColor = c === a ? "#c1b085" : "rgba(193,176,133,0.6)";
      box.style.boxShadow = c === a
        ? "0 0 16px rgba(193,176,133,0.35)"
        : "0 0 8px rgba(193,176,133,0.18)";
      box.style.background = "rgba(193,176,133,0.04)";
      spans[0].style.color = c === a ? "#c1b085" : "#b8984e";
    } else {
      box.style.borderColor = "#342a1c";
      box.style.boxShadow = "none";
      box.style.background = "rgba(193,176,133,0.02)";
      spans[0].style.color = "#6b5a38";
    }
  }
      ...
      (function (ii) {
        ...
        if (cb && !cb.dataset.bound) {
          ...
          cb.addEventListener("change", function () {
            if (cb.checked && na) na.checked = false;
            updatePage1State();
          });
        }
        ...
        if (na && !na.dataset.bound) {
          ...
          na.addEventListener("change", function () {
            if (na.checked && cb) cb.checked = false;
            updatePage1State();
          });
        }
      })(i);
  ...
  function progressHTML(active) {
    ...
    for (var i = 0; i < 7; i++) {
      h += '<div style="width:34px;height:2px;background:' +
        (i <= active ? "#c1b085" : "rgba(193,176,133,0.15)") +
        ';"></div>';
    }
    return h + "</div>";
  }
  ...
  function rowHTML(pi, ii) {
    var checked = ST[pi][ii] === 1;
    var na = ST[pi][ii] === -1;

    return ''
      + '<div class="la-item-wrap ' + (na ? 'is-na' : '') + '" style="display:flex;align-items:center;justify-content:space-between;gap:12px;">'
      +   '<div style="flex:1;">'
      +     '<div class="larow" onclick="window.__la.pick(' + pi + ',' + ii + ',\'check\')" style="border-color:' + (checked ? 'rgba(193,176,133,0.12)' : 'transparent') + ';background:' + (checked ? 'rgba(193,176,133,0.03)' : 'transparent') + ';opacity:' + (na ? '0.4' : '1') + ';filter:' + (na ? 'grayscale(100%)' : 'none') + ';pointer-events:' + (na ? 'none' : 'auto') + ';">'
      +       '<div class="lash" style="border-color:' + (checked ? '#c1b085' : '#7A6842') + ';box-shadow:' + (checked ? '0 0 12px rgba(193,176,133,0.6), 0 0 24px rgba(193,176,133,0.25), inset 0 0 8px rgba(193,176,133,0.1)' : 'none') + ';">'
      +         '<svg class="lamk" width="14" height="11" viewBox="0 0 14 11" fill="none" style="opacity:' + (checked ? '1' : '0') + ';transform:' + (checked ? 'scale(1)' : 'scale(0.6)') + ';">'
      +           '<path d="M1.5 5.5L5.5 9.5L12.5 1.5" stroke="#c1b085" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>'
      +         '</svg>'
      +       '</div>'
      +       '<span class="lalb" style="color:' + (checked ? '#c1b085' : '#9a8d7a') + ';' + (checked ? 'text-shadow:0 0 12px rgba(193,176,133,0.3);' : '') + '">' + P[pi].i[ii] + '</span>'
      +     '</div>'
      +   '</div>'
      +   '<div>'
      +     '<button type="button" class="lana-btn" onclick="window.__la.pick(' + pi + ',' + ii + ',\'na\')" style="color:' + (na ? '#c1b085' : '#7A6842') + ';border-color:' + (na ? '#7A6842' : '#342a1c') + ';background:' + (na ? '#342a1c' : 'transparent') + ';">N/A</button>'
      +   '</div>'
      + '</div>';
  }
  ...
  function businessHTML() {
    ...
    return ''
      ...
      +     '<button type="button" onclick="window.__la.ob(true)" style="background:' + (yesOn ? '#c1b085' : 'transparent') + ';color:' + (yesOn ? '#100d0a' : '#c1b085') + ';border:1px solid ' + (yesOn ? '#c1b085' : '#7A6842') + ';padding:12px 26px;font-family:Cinzel,serif;font-size:13px;letter-spacing:2px;cursor:pointer;box-shadow:' + (yesOn ? glow : 'none') + ';">YES</button>'
      +     '<button type="button" onclick="window.__la.ob(false)" style="background:' + (noOn ? '#c1b085' : 'transparent') + ';color:' + (noOn ? '#100d0a' : '#c1b085') + ';border:1px solid ' + (noOn ? '#c1b085' : '#7A6842') + ';padding:12px 26px;font-family:Cinzel,serif;font-size:13px;letter-spacing:2px;cursor:pointer;box-shadow:' + (noOn ? glow : 'none') + ';">NO</button>'
      +   '</div>'
  ...
  function pillarHTML(pi) {
    var rows = '';
    for (var i = 0; i < 6; i++) rows += rowHTML(pi, i);

    var backTarget = pi === 1 ? 1 : (pi === 6 && OB === false ? 5 : pi);
    var nextLabel = pi === 4 ? 'CONTINUE' : (pi === 6 ? 'SEE RESULTS' : 'NEXT PILLAR');
    ...
    return ''
      ...
      +   (pi === 0 ? '<span></span>' : '<button onclick="window.__la.bk(' + backTarget + ')" style="background:none;border:none;color:#b8984e;font-family:Cinzel,serif;font-size:14px;letter-spacing:3px;cursor:pointer;">BACK</button>')
      +   '<button onclick="window.__la.go(' + (pi + 2) + ')" style="background:#c1b085;border:none;color:#100d0a;padding:15px 36px;font-family:Cinzel,serif;font-size:15px;font-weight:700;letter-spacing:3px;cursor:pointer;">' + nextLabel + '</button>'
      + '</div>';
  }
  ...
  function detailHTML() {
    ...
    for (var pi = 0; pi < 7; pi++) {
      ...
      h += '<div style="margin-bottom:24px;">'
        ...
        + P[pi].n.toUpperCase() + ' ' + checkedCount(pi) + '/' + activeCount(pi)
        + '</div>';
  ...
  function resultsHTML() {
    ...
    for (var idx = 0; idx < 7; idx++) {
      ...
      var c = checkedCount(idx);
      var a = activeCount(idx);
      var w = a > 0 ? Math.round((c / a) * 100) : 100;
      ...
      brows += '<div style="display:flex;align-items:center;justify-content:space-between;gap:18px;padding:16px 0;border-bottom:1px solid rgba(193,176,133,0.10);">'
        ...
        +     '<div style="position:absolute;left:0;top:0;height:100%;width:' + w + '%;background:' + (c === a && a > 0 ? '#c1b085' : '#7A6842') + ';"></div>'
        +   '</div>'
        +   '<div style="font-family:Bodoni Moda,serif;font-size:15px;font-style:italic;color:#c1b085;min-width:32px;text-align:right;">' + c + '/' + a + '</div>'
        + '</div>'
        + '</div>';
    }
  ...
  function showRest(html) {
    ...
    if (!rest) return;
    rest.innerHTML = html;
    rest.style.animation = "none";
    void rest.offsetWidth;
    rest.style.animation = "la-in 0.4s ease";
    scrollToAudit();
  }
